        ...
        for cur_epoch in tbar:
            ...
            if test_loader is not None and (trained_epoch % ckpt_save_interval == 0 or trained_epoch in [1, 2, 4] or trained_epoch > total_epochs - 10):
                ...
                if cfg.LOCAL_RANK == 0:
                    ...
                    metric_name = cfg.get('MODEL_SELECTION_METRIC', None)
                    metric_mode = cfg.get('MODEL_SELECTION_MODE', None)

                    if metric_name in [None, 'auto']:
                        if 'mAP' in tb_dict:
                            metric_name, metric_mode = 'mAP', 'max'
                        elif 'minFDE' in tb_dict:
                            metric_name, metric_mode = 'minFDE', 'min'
                        elif 'minADE' in tb_dict:
                            metric_name, metric_mode = 'minADE', 'min'
                        elif 'MissRate' in tb_dict:
                            metric_name, metric_mode = 'MissRate', 'min'
                        else:
                            metric_name = None

                    if metric_name is None or metric_name not in tb_dict:
                        logger.info('Skip best-model selection because no valid metric is available in eval results.')
                        continue

                    if metric_mode is None:
                        metric_mode = 'max' if metric_name.lower() in ['map', 'accuracy', 'auc'] else 'min'
                    ...
                    try:
                        with open(best_record_file, 'r') as f:
                            best_src_data = [x.strip() for x in f.readlines() if x.strip() != '']
                        if len(best_src_data) > 0:
                            best_record_line = best_src_data[-1]
                            best_performance = float(best_record_line.split(' ')[-1])
                    except:
                        with open(best_record_file, 'a') as f:
                            pass

                    cur_metric = float(tb_dict[metric_name])
                    with open(best_record_file, 'a') as f:
                        print(f'epoch_{trained_epoch} {metric_name} {cur_metric}', file=f)

                    is_better = False
                    if best_performance is None:
                        is_better = True
                    elif metric_mode == 'max':
                        is_better = cur_metric > best_performance
                    else:
                        is_better = cur_metric < best_performance
                    ...
                    if is_better:
                        ...
                        logger.info(f'Save best model to {ckpt_name} by {metric_name} ({metric_mode})={cur_metric}')
                        with open(best_record_file, 'a') as f:
                            print(f'best_epoch_{trained_epoch} {metric_name} {cur_metric}', file=f)
                    elif best_record_line != '':
                        with open(best_record_file, 'a') as f:
                            print(best_record_line, file=f)
