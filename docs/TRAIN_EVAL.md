...
## Training & Testing on INTERACTION (history=1s, future=8s)

Train:
```
cd tools
python train.py --cfg_file cfgs/interaction/mtr_interaction_h1s_f8s.yaml --extra_tag interaction_h1s_f8s
```

Test:
```
cd tools
python test.py --cfg_file cfgs/interaction/mtr_interaction_h1s_f8s.yaml --ckpt ../output/interaction/mtr_interaction_h1s_f8s/interaction_h1s_f8s/ckpt/best_model.pth --extra_tag interaction_h1s_f8s
```
